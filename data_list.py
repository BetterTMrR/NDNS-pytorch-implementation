...
class ImageList_idx(Dataset):
    def __init__(self, image_list, root, labels=None, transform=None, strong_transform=None, target_transform=None, exist=None, mode='RGB'):
    ...
    def __getitem__(self, index):
        ...
        if self.strong_transform is not None:
            img_trans = self.strong_transform(img_o)
            return img, img_trans, int(target), index, self.exist[index], path

        return img, int(target), index, self.exist[index], path

    def __len__(self):
        return len(self.imgs)

    def add_item(self, imgs, labels, exist):
        self.imgs = np.concatenate((self.imgs, imgs), axis=0)
        self.labels = np.concatenate((self.labels, labels), axis=0)
        self.exist = np.concatenate((self.exist, exist), axis=0)
        return self.imgs, self.labels
...
def data_load(args):
    ...
    dset_loaders["tar_te"] = DataLoader(dsets["tar_te"], batch_size=train_bs * 2, shuffle=False, num_workers=args.worker,
                                          drop_last=False)
    return dset_loaders
...
def data_load_target(args):
    ...
    txt_src = open(args.s_dset_path).readlines()
    dsets["src_tr"] = ImageList_idx(txt_src, root=args.root, transform=image_train(alexnet=args.net=='alexnet'))
    dset_loaders["src_tr"] = DataLoader(dsets["src_tr"], batch_size=train_bs // 2, shuffle=True, num_workers=args.worker, drop_last=False)
    if args.ssda:
        txt_tar_un = open(args.t_dset_path_un).readlines()
        txt_tar_lbd = open(args.t_dset_path_lbd).readlines()
        dsets["tar_un"] = ImageList_idx(txt_tar_un, root=args.root, transform=image_train(alexnet=args.net=='alexnet'),
                                        strong_transform=image_strong(alexnet=args.net=='alexnet'))
        dsets["tar_test"] = ImageList_idx(txt_tar_un, root=args.root, transform=image_test(alexnet=args.net=='alexnet'))
        dsets['tar_lbd'] = ImageList_idx(txt_tar_lbd, root=args.root, transform=image_train(alexnet=args.net=='alexnet'))
        dset_loaders["tar_un"] = DataLoader(dsets["tar_un"], batch_size=train_bs, shuffle=True, num_workers=args.worker,
                                            drop_last=False)
        dset_loaders["tar_test"] = DataLoader(dsets["tar_test"], batch_size=train_bs * 3, shuffle=False,
                                              num_workers=args.worker,
                                              drop_last=False)
        dset_loaders['tar_lbd'] = DataLoader(dsets['tar_lbd'], batch_size=train_bs // 2, shuffle=True,
                                              num_workers=args.worker,
                                              drop_last=False)
    else:
